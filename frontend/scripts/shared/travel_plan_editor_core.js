...
export function createBookingTravelPlanModule(ctx) {
  ...
  function travelPlanRenumberDaysButton() {
    const button = document.getElementById("travel_plan_renumber_days_btn") || els.travel_plan_renumber_days_btn;
    return button instanceof HTMLButtonElement ? button : null;
  }

  function syncTravelPlanRenumberDaysAction() {
    const button = travelPlanRenumberDaysButton();
    if (!button) return;
    ...
    button.textContent = bookingT("booking.travel_plan.renumber_days", "Renumber Dates");
  ...
  function timeValueFromTravelPlanTime(value, dayDate = "") {
    const raw = String(value || "").trim();
    if (/^\d{2}:\d{2}$/.test(raw)) return raw;
    return splitDateTimeValue(dayDate, raw).time;
  }
  ...
  function applyTravelPlanDatePickerValue(pickerInput) {
    ...
    clearTravelPlanDateString(textInput);
  ...
  function dateStringInputForDateTextInput(textInput) {
    const wrapper = textInput?.closest(".travel-plan-date-input");
    return wrapper?.querySelector('[data-travel-plan-day-field="date_string"]') || null;
  }

  function clearTravelPlanDateString(textInput) {
    if (!allowDates) return;
    const hiddenInput = dateStringInputForDateTextInput(textInput);
  ...
  function applyTravelPlanDayDateSelection(textInput, { date = "", dateString = "" } = {}) {
    ...
    syncTravelPlanDateShortcutButtons();
    return textInput;
  }
  ...
  function syncTravelPlanDateShortcutButtons() {
  ...
  function renderTravelPlanBoundaryTimingFields(boundaryKind, item) {
    ...
    const timingKind = String(item?.timing_kind || "label");
    const derivedDate = deriveTravelPlanBoundaryDate(boundaryKind);
    if (timingKind === "not_applicable") {
      return `
        <div class="field travel-plan-timing-field travel-plan-timing-field--kind">
    ...
    if (timingKind === "point") {
      const pointTime = timeValueFromTravelPlanTime(item.time_point, derivedDate);
      return `
        <div class="field travel-plan-timing-field travel-plan-timing-field--kind">
          <label for="travel_plan_boundary_timing_kind_${escapeHtml(idPart)}">${escapeHtml(bookingT("booking.travel_plan.time_information", "When?"))}</label>
          <select id="travel_plan_boundary_timing_kind_${escapeHtml(idPart)}" data-travel-plan-boundary-field="timing_kind">
        ...
        <div class="field travel-plan-timing-field travel-plan-timing-field--time">
          ...
          <select id="travel_plan_boundary_time_point_time_${escapeHtml(idPart)}" data-travel-plan-boundary-field="time_point_time">
            ${timeSelectOptions(pointTime)}
          </select>
        </div>
      `;
    }
    if (timingKind === "range") {
      const startTime = timeValueFromTravelPlanTime(item.start_time, derivedDate);
      const endTime = timeValueFromTravelPlanTime(item.end_time, derivedDate);
      return `
        <div class="field travel-plan-timing-field travel-plan-timing-field--kind">
          <label for="travel_plan_boundary_timing_kind_${escapeHtml(idPart)}">${escapeHtml(bookingT("booking.travel_plan.time_information", "When?"))}</label>
          <select id="travel_plan_boundary_timing_kind_${escapeHtml(idPart)}" data-travel-plan-boundary-field="timing_kind">
  ...
  function travelPlanDayDateLabel(day) {
    ...
    if (date) return date;
    const dateString = normalizeTravelPlanDayDateString(day?.date_string);
    if (dateString === TRAVEL_PLAN_DAY_DATE_STRING.BEFORE_TRIP) {
      return bookingT("booking.travel_plan.date_string.before_trip", "Before the trip");
    }
    if (dateString === TRAVEL_PLAN_DAY_DATE_STRING.AFTER_TRIP) {
  ...
  function travelPlanBoundaryDisplay(boundaryKind) {
    const source = state.travelPlanDraft?.boundary_logistics?.[boundaryKind] || createEmptyTravelPlanBoundaryService(boundaryKind);
    const item = normalizeTravelPlanBoundaryDraft(source, boundaryKind);
  ...
  function deriveTravelPlanBoundaryDate(boundaryKind) {
    if (!allowDates) return "";
    const normalizedBoundaryKind = normalizeTravelPlanBoundaryKind(boundaryKind);
    const days = travelPlanDays();
    const source = state.travelPlanDraft?.boundary_logistics?.[normalizedBoundaryKind] || {};
    const presentation = defaultTravelPlanBoundaryPresentation(normalizedBoundaryKind, source?.presentation?.attach_to || "");
    const adjacentDay = normalizedBoundaryKind === "departure" ? days[days.length - 1] : days[0];
    const adjacentDate = String(adjacentDay?.date || "").trim();
    if (!isIsoDateString(adjacentDate)) return "";
    if (normalizedBoundaryKind === "departure") {
      return presentation.attach_to === "after_last_day" ? shiftIsoDate(adjacentDate, 1) : adjacentDate;
    }
    return presentation.attach_to === "before_first_day" ? shiftIsoDate(adjacentDate, -1) : adjacentDate;
  }
  ...
  function travelPlanCustomizerTrips() {
    ...
    tripsById.set(bookingTrip.id, { ...bookingTrip, dayIds: new Set() });
    ...
    return Array.from(tripsById.values()).map(({ dayIds: _dayIds, ...trip }) => ({
      ...
      travel_plan: {
        ...trip.travel_plan,
        days: [...(Array.isArray(trip.travel_plan?.days) ? trip.travel_plan.days : [])].sort((left, right) => {
  ...
  function renderTravelPlanCustomizerLauncher() {
    return `
      <div
        class="travel-plan-customizer-preview"
        role="button"
        tabindex="0"
  ...
  function renderTravelPlanRenumberDatesAction() {
    if (!allowRenumberDays) return "";
    return `
      <button
        class="btn btn-ghost travel-plan-renumber-dates-btn"
        id="travel_plan_renumber_days_btn"
        data-travel-plan-renumber-days
        type="button"
      >
        ${escapeHtml(bookingT("booking.travel_plan.renumber_days", "Renumber Dates"))}
      </button>
    `;
  }

  async function loadTravelPlanCustomizerSourceRows() {
    if (!useFocusedBookingWorkspace) return [];
  ...
  function renderTravelPlanCustomizerWorkspaces() {
    if (!useFocusedBookingWorkspace || !els.travel_plan_editor) return;
  ...
  function renderTravelPlanSidebar(selection) {
    ...
    return `
      <aside class="travel-plan-booking-sidebar" aria-label="Travel plan navigation">
        ...
        ${renderTravelPlanRenumberDatesAction()}
        <div class="travel-plan-route-list">
          <button class="travel-plan-route-list__item${arrivalSelected ? " is-selected" : ""}" data-travel-plan-select-boundary="arrival" type="button">
            ...
            <span class="travel-plan-route-list__copy">
              <strong>${escapeHtml(boundaryLabel("arrival"))}</strong>
              <small>${escapeHtml(travelPlanBoundaryDisplay("arrival"))}</small>
            </span>
          </button>
          ${days.map((day, index) => {
            ...
            return `
              <button class="travel-plan-route-list__item${selected ? " is-selected" : ""}" data-travel-plan-select-day="${escapeHtml(dayId)}" type="button">
                <span class="travel-plan-route-list__badge">${escapeHtml(String(index + 1))}</span>
                <span class="travel-plan-route-list__copy">
                  <strong>${escapeHtml(travelPlanDayTitle(day, index))}</strong>
                  <small>${escapeHtml(meta || bookingT("booking.travel_plan.location_none", "No map point"))}</small>
                </span>
              </button>
            `;
          }).join("")}
          <button class="travel-plan-route-list__item${departureSelected ? " is-selected" : ""}" data-travel-plan-select-boundary="departure" type="button">
            <span class="travel-plan-route-list__badge travel-plan-route-list__badge--image" aria-hidden="true">
              <img src="/assets/img/departure.png" alt="" loading="lazy" decoding="async" />
            </span>
            <span class="travel-plan-route-list__copy">
              <strong>${escapeHtml(boundaryLabel("departure"))}</strong>
              <small>${escapeHtml(travelPlanBoundaryDisplay("departure"))}</small>
            </span>
          </button>
        </div>
      </aside>
  ...
  function renderTravelPlanFocusedEditor(selection) {
        ...
        <section class="travel-plan-booking-editor" aria-label="${escapeHtml(title)}">
          <div class="travel-plan-booking-editor__toolbar">
            <div class="travel-plan-booking-editor__title">
              <span class="travel-plan-booking-editor__badge">${selection.boundaryKind === "arrival" ? "A" : "D"}</span>
              <div>
                <h3>${escapeHtml(title)}</h3>
                <p>${escapeHtml(travelPlanBoundaryDisplay(selection.boundaryKind))}</p>
              </div>
            </div>
            <button class="btn btn-ghost booking-offer-add-btn travel-plan-booking-editor__add-day" data-travel-plan-add-day type="button">
              <span class="travel-plan-add-btn__icon" aria-hidden="true">+</span>
    ...
    const meta = [travelPlanDayDateLabel(day), travelPlanDayLocationLabel(day)].filter(Boolean).join(" · ");
    return `
      <section class="travel-plan-booking-editor" aria-label="${escapeHtml(travelPlanDayTitle(day, dayIndex))}">
        <div class="travel-plan-booking-editor__toolbar">
          <div class="travel-plan-booking-editor__title">
            <span class="travel-plan-booking-editor__badge">${escapeHtml(String(dayIndex + 1))}</span>
            <div>
              <h3>${escapeHtml(travelPlanDayTitle(day, dayIndex))}</h3>
              <p>${escapeHtml(meta || bookingT("booking.travel_plan.location_none", "No map point"))}</p>
            </div>
          </div>
          <button class="btn btn-ghost booking-offer-add-btn travel-plan-booking-editor__add-day" data-travel-plan-add-day type="button">
            <span class="travel-plan-add-btn__icon" aria-hidden="true">+</span>
        ...
        ${renderTravelPlanDay(day, dayIndex)}
      </section>
    `;
  }
  ...
  function readTravelPlanBoundaryServiceFromDom(boundaryKind, previousBoundary) {
    ...
    if (allowTiming) {
      ...
      item.time_point = String(node?.querySelector('[data-travel-plan-boundary-field="time_point_time"]')?.value || "").trim();
      item.start_time = String(node?.querySelector('[data-travel-plan-boundary-field="start_time_time"]')?.value || "").trim();
      item.end_time = String(node?.querySelector('[data-travel-plan-boundary-field="end_time_time"]')?.value || "").trim();
    } else {
      item.time_label = "";
    ...
    if (els.travel_plan_editor && els.travel_plan_editor.dataset.travelPlanBound !== "true") {
      els.travel_plan_editor.addEventListener("input", (event) => {
        ...
        if (target?.matches?.('[data-travel-plan-date-text="true"]')) {
          clearTravelPlanDateString(target);
      ...
      els.travel_plan_editor.addEventListener("click", (event) => {
        ...
        if (!button) return;
        if (button.hasAttribute("data-travel-plan-renumber-days")) {
