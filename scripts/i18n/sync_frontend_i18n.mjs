...
function usage() {
  console.error(
    [
      "Usage:",
      "  node scripts/i18n/sync_frontend_i18n.mjs check [--source en] [--target vi]",
      "  node scripts/i18n/sync_frontend_i18n.mjs translate [--source en] [--target vi] [--force-all]",
      "",
      "Notes:",
...
async function readManualOverrides(source, targetLang) {
  ...
  for (const [key, sourceText] of Object.entries(source || {})) {
    ...
    const errors = validateTranslationManualOverride(item, {
      sourceText,
      context: `frontend/${targetLang}: ${key}`
    });
...
function printCheckSummary(targetLang, state) {
  ...
  console.error(`Run: node scripts/i18n/sync_frontend_i18n.mjs translate --target ${targetLang}`);
}
...
async function translateEntries(sourceEntries, targetLang, sourceLang, translatorSession) {
  ...
  const translatedMaskedEntries = await translatorSession.client.translateEntries(maskedEntries, targetLang, {
    ...
    onEntryComplete({ key, completedEntries, totalEntries }) {
      console.log(`Translated [${targetLang} ${completedEntries}/${totalEntries}] ${key}`);
    }
  });
...
async function runCheck(options) {
  ...
  for (const targetLang of targetLangs) {
    ...
    const overrides = await readManualOverrides(source, targetLang);
...
async function runTranslate(options) {
  ...
  for (const targetLang of targetLangs) {
    ...
    console.log(`Frontend target ${targetLang}: ${translationKeys.length} key${translationKeys.length === 1 ? "" : "s"} need machine translation.`);
