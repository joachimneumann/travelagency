...
ZOHO_MAIL_API_BASE_URL="${ZOHO_MAIL_API_BASE_URL:-https://mail.zoho.com}"
ZOHO_CLIENT_ID="${ZOHO_CLIENT_ID:-}"
ZOHO_CLIENT_SECRET="${ZOHO_CLIENT_SECRET:-}"
ZOHO_REFRESH_TOKEN="${ZOHO_REFRESH_TOKEN:-}"
ZOHO_ACCOUNT_ID="${ZOHO_ACCOUNT_ID:-}"
WEB_INQUIRY_NOTIFICATION_ENABLED="${WEB_INQUIRY_NOTIFICATION_ENABLED:-true}"
WEB_INQUIRY_NOTIFICATION_FROM="${WEB_INQUIRY_NOTIFICATION_FROM:-noreply@asiatravelplan.com}"
WEB_INQUIRY_NOTIFICATION_TO="${WEB_INQUIRY_NOTIFICATION_TO:-booking@asiatravelplan.com}"
WEB_INQUIRY_NOTIFICATION_BACKEND_BASE_URL="${WEB_INQUIRY_NOTIFICATION_BACKEND_BASE_URL:-http://127.0.0.1:${FRONTEND_PORT}}"
WEB_INQUIRY_NOTIFICATION_LOGO_URL="${WEB_INQUIRY_NOTIFICATION_LOGO_URL:-${WEB_INQUIRY_NOTIFICATION_BACKEND_BASE_URL%/}/assets/generated/runtime/brand-logo.png}"
PUBLIC_SITE_RUNTIME_BRAND_ENV="${PUBLIC_SITE_RUNTIME_BRAND_ENV:-local}"
...
TRAVELER_DETAILS_TOKEN_TTL_SECONDS="${TRAVELER_DETAILS_TOKEN_TTL_SECONDS:-86400}"
CORS_ORIGIN="${CORS_ORIGIN:-http://localhost:${FRONTEND_PORT},http://127.0.0.1:${FRONTEND_PORT},http://localhost,http://127.0.0.1}"
RETURN_TO_ALLOWED_ORIGINS="${RETURN_TO_ALLOWED_ORIGINS:-http://localhost:${FRONTEND_PORT},http://127.0.0.1:${FRONTEND_PORT},http://localhost:${BACKEND_PORT},http://127.0.0.1:${BACKEND_PORT}}"
...
EOF
  ...
  env \
    ...
    ZOHO_MAIL_API_BASE_URL="$ZOHO_MAIL_API_BASE_URL" \
    ZOHO_CLIENT_ID="$ZOHO_CLIENT_ID" \
    ZOHO_CLIENT_SECRET="$ZOHO_CLIENT_SECRET" \
    ZOHO_REFRESH_TOKEN="$ZOHO_REFRESH_TOKEN" \
    ZOHO_ACCOUNT_ID="$ZOHO_ACCOUNT_ID" \
    WEB_INQUIRY_NOTIFICATION_ENABLED="$WEB_INQUIRY_NOTIFICATION_ENABLED" \
    WEB_INQUIRY_NOTIFICATION_FROM="$WEB_INQUIRY_NOTIFICATION_FROM" \
    WEB_INQUIRY_NOTIFICATION_TO="$WEB_INQUIRY_NOTIFICATION_TO" \
    WEB_INQUIRY_NOTIFICATION_BACKEND_BASE_URL="$WEB_INQUIRY_NOTIFICATION_BACKEND_BASE_URL" \
    WEB_INQUIRY_NOTIFICATION_LOGO_URL="$WEB_INQUIRY_NOTIFICATION_LOGO_URL" \
    PUBLIC_SITE_RUNTIME_BRAND_ENV="$PUBLIC_SITE_RUNTIME_BRAND_ENV" \
    OPENAI_API_KEY="$OPENAI_API_KEY" \
    OPENAI_PROJECT_ID="$OPENAI_PROJECT_ID" \
    OPENAI_ORGANIZATION_ID="$OPENAI_ORGANIZATION_ID" \
    ...
    BOOKING_CONFIRMATION_TOKEN_SECRET="$BOOKING_CONFIRMATION_TOKEN_SECRET" \
    BOOKING_CONFIRMATION_TOKEN_TTL_SECONDS="$BOOKING_CONFIRMATION_TOKEN_TTL_SECONDS" \
    TRAVELER_DETAILS_TOKEN_SECRET="$TRAVELER_DETAILS_TOKEN_SECRET" \
    TRAVELER_DETAILS_TOKEN_TTL_SECONDS="$TRAVELER_DETAILS_TOKEN_TTL_SECONDS" \
    node <<'EOF'
const fs = require("fs");
