...
import_zsh_env() {
  ...
  local exported
  exported="$(zsh -ilc 'source ~/.zshrc >/dev/null 2>&1 || true; typeset -px KEYCLOAK_ENABLED KEYCLOAK_BASE_URL KEYCLOAK_REALM KEYCLOAK_CLIENT_ID KEYCLOAK_CLIENT_SECRET KEYCLOAK_REDIRECT_URI KEYCLOAK_ALLOWED_ROLES KEYCLOAK_POST_LOGOUT_REDIRECT_URI GOOGLE_SERVICE_ACCOUNT_JSON_PATH GOOGLE_IMPERSONATED_EMAIL OPENAI_API_KEY OPENAI_TRANSLATION_MODEL OPENAI_MODEL BOOKING_CONFIRMATION_TOKEN_SECRET BOOKING_CONFIRMATION_TOKEN_TTL_SECONDS CORS_ORIGIN FRONTEND_PORT BACKEND_PORT 2>/dev/null' 2>/dev/null || true)"
  [ -n "$exported" ] || return 0
  eval "$exported"
}
