...
function usage() {
  console.error(
    [
      "Usage:",
      "  node scripts/sync_backend_i18n.mjs check [--source en] [--target vi]",
      "  node scripts/sync_backend_i18n.mjs translate [--source en] [--target vi] [--force-all]"
    ].join("\n")
  );
}
...
function printCheckSummary(targetLang, state) {
  ...
  console.error(`Run: node scripts/sync_backend_i18n.mjs translate --target ${targetLang}`);
}
...
async function runTranslate(options) {
  ...
  await writeJsonFile(dictionaryPath(options.targetLang), orderedObjectFromSourceKeys(source, nextTarget));
  await writeJsonFile(metadataPath(options.targetLang), orderedObjectFromSourceKeys(source, nextMeta));
