...
import_zsh_env() {
  ...
  local exported
  exported="$(zsh -ilc 'typeset -px KEYCLOAK_ENABLED KEYCLOAK_BASE_URL KEYCLOAK_REALM KEYCLOAK_CLIENT_ID KEYCLOAK_CLIENT_SECRET KEYCLOAK_REDIRECT_URI KEYCLOAK_ALLOWED_ROLES KEYCLOAK_POST_LOGOUT_REDIRECT_URI KEYCLOAK_ADMIN KEYCLOAK_ADMIN_PASSWORD KEYCLOAK_DIRECTORY_USERNAME KEYCLOAK_DIRECTORY_PASSWORD KEYCLOAK_DIRECTORY_ADMIN_REALM CORS_ORIGIN FRONTEND_PORT BACKEND_PORT 2>/dev/null' 2>/dev/null || true)"
  [ -n "$exported" ] || return 0
  eval "$exported"
}
