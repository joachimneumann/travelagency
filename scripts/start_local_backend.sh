...
import_zsh_env() {
  ...
  local exported
  exported="$(zsh -ilc 'source ~/.zshrc >/dev/null 2>&1 || true; typeset -px KEYCLOAK_ENABLED KEYCLOAK_BASE_URL KEYCLOAK_REALM KEYCLOAK_CLIENT_ID KEYCLOAK_CLIENT_SECRET KEYCLOAK_REDIRECT_URI KEYCLOAK_ALLOWED_ROLES KEYCLOAK_POST_LOGOUT_REDIRECT_URI KEYCLOAK_ADMIN KEYCLOAK_ADMIN_PASSWORD KEYCLOAK_DIRECTORY_USERNAME KEYCLOAK_DIRECTORY_PASSWORD KEYCLOAK_DIRECTORY_ADMIN_REALM GOOGLE_SERVICE_ACCOUNT_JSON_PATH GOOGLE_IMPERSONATED_EMAIL OPENAI_API_KEY OPENAI_TRANSLATION_MODEL OPENAI_MODEL OFFER_ACCEPTANCE_TOKEN_SECRET OFFER_ACCEPTANCE_TOKEN_TTL_SECONDS CORS_ORIGIN FRONTEND_PORT BACKEND_PORT WHATSAPP_WEBHOOK_ENABLED WHATSAPP_VERIFY_TOKEN WHATSAPP_WEBHOOK_VERIFY_TOKEN WHATSAPP_APP_SECRET WHATSAPP_WEBHOOK_APP_SECRET META_WEBHOOK_ENABLED META_WEBHOOK_VERIFY_TOKEN META_APP_SECRET 2>/dev/null' 2>/dev/null || true)"
  [ -n "$exported" ] || return 0
  eval "$exported"
}
...
OFFER_ACCEPTANCE_TOKEN_SECRET="${OFFER_ACCEPTANCE_TOKEN_SECRET:-}"
OFFER_ACCEPTANCE_TOKEN_TTL_SECONDS="${OFFER_ACCEPTANCE_TOKEN_TTL_SECONDS:-}"
