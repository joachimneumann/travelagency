...
test("homepage tour cards expand descriptions and align same-row cards without an inline more link", async () => {
  ...
  assert.match(
    mainToursSource,
    /function cancelActiveTourDetailsAnimations\(\) \{[\s\S]*function beginTourDetailsTransition\(\)[\s\S]*function animateTourDetailsToggle\(tripId, willOpen\)[\s\S]*async function animateTourDetailsOpen\(tripId, transitionToken\)[\s\S]*renderVisibleTrips\(\);[\s\S]*applyTourCardMediaSnapshots\(previousMediaSnapshots\);[\s\S]*const rowClearingPromise = singleColumnLayout[\s\S]*\? Promise\.resolve\(\)[\s\S]*: Promise\.all\(\[[\s\S]*animateTourGridLayout\(previousRects, \{ excludedTripIds: \[tripId\] \}\)[\s\S]*animateExpandedTourCardToLeft\(row\)[\s\S]*\]\);[\s\S]*const opensSideways = !singleColumnLayout && row\.classList\.contains\("tour-details-row--side-panel"\);[\s\S]*const startDetailsOpenAnimation = \(targetHeight\) => Promise\.all\(\[[\s\S]*animateExpandedTourDetailsAttach\(row, TOUR_DETAILS_OPEN_TRANSITION_MS\)[\s\S]*animateTourDetailsRowHeight\(row, opensSideways \? collapsedHeight : targetHeight, "open"\)[\s\S]*const detailsOpenPromise = opensSideways[\s\S]*\? startDetailsOpenAnimation\(collapsedHeight\)[\s\S]*: null;[\s\S]*await waitForExpandedTourServiceImages\(row\);[\s\S]*if \(singleColumnLayout\) \{[\s\S]*TOUR_DETAILS_MOBILE_OPEN_EASING[\s\S]*clearTourDetailsRowAnimation\(row\);[\s\S]*completeTourDetailsTransition\(transitionToken, tripId\);[\s\S]*return;[\s\S]*\}[\s\S]*await rowClearingPromise;[\s\S]*detailsOpenPromise \|\| startDetailsOpenAnimation\(expandedHeight\)[\s\S]*clearTourDetailsRowAnimation\(row, \{ preserveHeight: opensSideways \}\);[\s\S]*completeTourDetailsTransition\(transitionToken, tripId\);[\s\S]*async function animateTourDetailsClose\(tripId, transitionToken\)[\s\S]*const outgoingDetailsGhost = createOutgoingTourDetailsGhost\(row\);[\s\S]*renderVisibleTrips\(\);[\s\S]*applyTourCardMediaSnapshots\(previousMediaSnapshots\);[\s\S]*await Promise\.all\(\[\s*animateOutgoingTourDetailsGhost\(outgoingDetailsGhost\),\s*animateTourGridLayout\(previousRects\),\s*animateTourShowMoreButtonLabel\(\s*closedButton,\s*tourShowMoreLabel\(false\),\s*\{ direction: "close" \}\s*\)\s*\]\);[\s\S]*completeTourDetailsTransition\(transitionToken, tripId\);/,
