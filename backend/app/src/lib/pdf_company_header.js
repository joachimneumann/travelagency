...
export function drawPdfCompanyHeader(doc, {
  ...
  if (logoSource) {
    if (logoWidthOnly) {
      doc.image(logoSource, pageMargin, y + 2, { width: logoWidth });
    } else {
  ...
  if (!hideRightColumn) {
    ...
    const websiteHeight = profile.website
      ...
      : 0;
    rightColumnBottomY = Math.max(websiteY + websiteHeight, emailY + 12, addressY + addressHeight);
  }
